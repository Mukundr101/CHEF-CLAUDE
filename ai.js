...
export async function getRecipeFromMistral(ingredientsArr) {
    ...
    const cuisineStyles = ["Italian", "Mexican", "Asian", "Mediterranean", "American", "Indian", "French", "Thai"];
    const randomCuisine = cuisineStyles[Math.floor(Math.random() * cuisineStyles.length)];
    
    const systemPrompt = `You are a creative chef assistant specializing in ${randomCuisine} cuisine. Create delicious recipes based on the ingredients provided. Format your response in markdown with a recipe title, ingredients list with quantities, and step-by-step instructions.`;
    
    const userPrompt = `I have the following ingredients: ${ingredientsString}.

Please suggest a delicious ${randomCuisine}-inspired recipe I can make using some or all of these ingredients. You can suggest a few additional common ingredients if needed.
